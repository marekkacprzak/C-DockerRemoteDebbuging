...
exit 0
{
    ...
    # Error occurred, cleanup the compressed binary and exit with error.
    if [ -n "$errorMessage" ]; then
        fail "$errorMessage"
    fi
}
...
if [ "$__UseAltDebuggerLocation" = false ]; then
    if [ "$__SkipDownloads" = true ]; then
        ...
        if [ -z "$__VsdbgCompressedFile" ]; then
            ...
            echo "Info: Using Runtime ID '$__RuntimeID'"
            # Download manually and place the vsdbg compressed file in the script directory
            # Replace <URL> with the actual URL to download the vsdbg compressed file
            curl -x <proxy_address:port> -o vsdbg.tar.gz <URL>
        fi
